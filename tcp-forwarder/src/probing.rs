...
/// 探测任务：周期性地探测IP并更新评分
#[instrument(skip(score_board, config))]
pub async fn probing_task(score_board: ScoreBoard, config: RemotesConfig) -> anyhow::Result<()> {
    ...
    loop {
        ...
        // 选择需要探测的IP
        let candidates =
            select_probe_candidates(&score_board, config.probing.probe_candidate_count);
...
/// 从评分板中选择需要探测的候选IP
fn select_probe_candidates(score_board: &ScoreBoard, count: usize) -> Vec<(IpAddr, ScoreData)> {
    // 从评分板中获取所有IP
